...
void read_matrix(ifstream& read, int** mat, int rows, int cols) {
	for (int i = 0; i < rows; ++i)
		for (int j = 0; j < cols; ++j) read >> mat[i][j];
}
void print_matrix(int** mat, int rows, int cols) {
	for (int i = 0; i < rows; ++i) {
...
void add_matrices() {
	...
	for (int i = 0; i < r1; ++i)
		for (int j = 0; j < c1; ++j) sum[i][j] = a[i][j] + b[i][j];
...
void display_diagonal() {
	...
	for (int i = 0; i < r && i < c; ++i) cout << mat[i][i] << " ";
	cout << endl;
...
void dot_cross_product() {
	...
	if (r1 == 1 && r2 == 1 && c1 == c2 && c1 == 3) {
		int dot = 0;
		for (int i = 0; i < 3; ++i) dot += a[0][i] * b[0][i];
		...
		cout << "Cross product: ";
		for (int i = 0; i < 3; ++i) cout << cross[i] << " ";
		cout << endl;
	} else {
...
void create_jagged_array() {
	...
	for (int i = 0; i < rows; ++i) {
		for (int j = 0; j < sizes[i]; ++j) cout << jagged[i][j] << " ";
		cout << endl;
	}

	for (int i = 0; i < rows; ++i) delete[] jagged[i];
	delete[] jagged;
	delete[] sizes;
}
void cinema_jagged_array() {
	...
	for (int i = 0; i < rows; ++i) {
		...
		for (int j = 0; j < seat_sizes[i]; ++j) seats[i][j] = 0;  // 0 means empty
	}
	...
	for (int i = 0; i < rows; ++i) {
		char row_label = 'A' + i;
		cout << row_label << ": ";
		for (int j = 0; j < seat_sizes[i]; ++j) cout << seats[i][j] << " ";
		cout << endl;
	}
